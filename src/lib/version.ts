...
/** One-time console diagnostics: log the portal build + the API build, warn on mismatch. */
export function logVersionDiagnostics(): void {
  const b = buildTime ? ` · built ${buildTime}` : '';
  console.info(`Worktide Portal ${version} · ${shortCommit}${b}`);
  ...
  api
    ...
    .then(({ data }) => {
      ...
      if (
        ...
        console.warn(
          `Version mismatch — portal ${shortCommit} vs API ${data.shortCommit}. A hard refresh may be needed.`,
        );
      }
    })
    .catch(() => {
